<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="教练等级" prop="jiaoliandengji">
          <el-input v-model="ruleForm.jiaoliandengji" 
              placeholder="教练等级" clearable  :readonly="ro.jiaoliandengji"></el-input>
        </el-form-item>
        <div v-else>
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.jiaoliantupian" label="教练图片" prop="jiaoliantupian">
          <file-upload
          tip="点击上传教练图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.jiaoliantupian?ruleForm.jiaoliantupian:''"
          @change="jiaoliantupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.jiaoliantupian" label="教练图片" prop="jiaoliantupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jiaoliantupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jiaolianjieshao = true;
          ...
          if(o=='jiaoliantupian'){
            this.ruleForm.jiaoliantupian = obj[o];
	    this.ro.jiaoliantupian = true;
            continue;
          }
          if(o=='thumbsupnum'){
      ...
      this.$http({
		...
		if(json.jiaoliandengji!=''&&json.jiaoliandengji){
                this.ruleForm.jiaoliandengji = json.jiaoliandengji
	    		this.ro.jiaoliandengji = true;
		}
		if(json.lianxidianhua!=''&&json.lianxidianhua){
          ...
          this.$message.error(data.msg);
            ...
            this.yuyuezhuangtaiOptions = "繁忙,空闲".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
	...
	this.ruleForm.jiaolianjieshao = this.ruleForm.jiaolianjieshao.replace(reg,'../../../springboot6hv37/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.jiaoliantupian!=null) {
		this.ruleForm.jiaoliantupian = this.ruleForm.jiaoliantupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    jiaoliantupianUploadChange(fileUrls) {
	this.ruleForm.jiaoliantupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
